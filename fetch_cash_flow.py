...
def get_prices(ticker):
    ...
    recent_years = stock.cash_flow.columns[-4:][::-1] # Get the last 4 columns
    financial_metrics = {}
    metrics_of_interest = ["Operating Cash Flow", "Investing Cash Flow", "Financing Cash Flow", "Free Cash Flow"]
    ...
    for metric in metrics_of_interest:
        for year in range(4):
            ...
            try:
                ...
                if metric in stock.cash_flow.index:
                    financial_metrics[year_label] = stock.cash_flow.loc[metric, year_col]
                else:
    ...
    return {
        ...
        **financial_metrics,  # Expand the revenue_data dictionary into the return dictionary
        "Price to Earnings PE": trailingPE,
        "Price to Book PB": priceToBook
